...
class _DashBoardMain(QWidget):
    ...
    def __init__(self, parent, size: Union[tuple, list] = (1366, 768), hide_creator_button: bool = True,
        ...
        self.keys_ = {Qt.Key.Key_W: False, Qt.Key.Key_H: False, Qt.Key.Key_D: False, Qt.Key.Key_A: False, Qt.Key.Key_Left: False, 
                        Qt.Key.Key_Right: False, Qt.Key.Key_Space: False, Qt.Key.Key_Escape: False,Qt.Key.Key_T: False,
                        Qt.Key.Key_Z: False}
    ...
    def customKeyPressEvent(self, event):
        ...
        # indicator1
        if event.key() == Qt.Key.Key_A or self.keys_[Qt.Key.Key_A]:
            self.keys_[event.key()] = True

        # indicator2
        if event.key() == Qt.Key.Key_D or self.keys_[Qt.Key.Key_D]:
            self.keys_[event.key()] = True

        # headLight
        if event.key() == Qt.Key.Key_T or self.keys_[Qt.Key.Key_T]:
            self.keys_[event.key()] = True

        if event.key() == Qt.Key.Key_Z or self.keys_[Qt.Key.Key_Z]:
            self.keys_[event.key()] = True    
    ...
    def customKeyReleaseEvent(self, event):
        ...
        if event.key() == Qt.Key.Key_A and not event.isAutoRepeat():
            self.keys_[event.key()] = False
            self.dash_board_design_widget.set_indicator1_state(0)

        # indicator2
        if event.key() == Qt.Key.Key_D and not event.isAutoRepeat():
            self.keys_[event.key()] = False
            self.dash_board_design_widget.set_indicator2_state(0)

        # headLight
        if event.key() == Qt.Key.Key_T and not event.isAutoRepeat():
            self.keys_[event.key()] = False
            self.dash_board_design_widget.set_headLight_state(0)

        if event.key() == Qt.Key.Key_Z and not event.isAutoRepeat():
            self.keys_[event.key()] = False
            self.dash_board_design_widget.set_headLight2_state(0)
    ...
    def keyAction(self):
        if any(self.keys_.values()):
            ...
            # indicator1
            if self.keys_[Qt.Key.Key_A]:
                self.dash_board_design_widget.set_indicator1_state(1)

            # indicator2
            if self.keys_[Qt.Key.Key_D]:
                self.dash_board_design_widget.set_indicator2_state(1)

            # headLight
            if self.keys_[Qt.Key.Key_T]:
                self.dash_board_design_widget.set_headLight_state(1)

            if self.keys_[Qt.Key.Key_Z]:
                self.dash_board_design_widget.set_headLight2_state(1)    
...
class _DashBoardContolsDesign(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.indicator1_properties()
        self.indicator2_properties()
        self.headLight_properties()
        self.headLight2_properties()
    ...
    def indicators_properties(self):
        ...
        self.indicator_color_list = (QColorConstants.DarkGreen, QColorConstants.Red)
    ...
    def indicator_triger(self, indecator):
        ...
        if self.right_indicator_state or self.left_indicator_state:
            self.indicator_timer.start(500) # blink indicator interval of 600ms
        else:
    ...
    def indicator1_properties(self):
        self.indicator1_lst = (QColor(67, 13, 13, 200), QColorConstants.Green)
        self.indicator1_color_idx = 0
        self.indicator1_state = 0 # 0 -> off 1 -> on

    def indicator2_properties(self):
        self.indicator2_lst = (QColor(67, 13, 13, 200), QColorConstants.Green)
        self.indicator2_color_idx = 0
        self.indicator2_state = 0 # 0 -> off 1 -> on

    def headLight_properties(self):
        self.headLight_sound_color_lst = (QColor(67, 13, 13, 200), QGradient(QGradient.Preset.HeavyRain))
        self.headLight_sound_color_idx = 0
        self.headLight_state = 0 # 0 -> off 1 -> on

    def headLight2_properties(self):
        self.headLight2_sound_color_lst = (QColor(67, 13, 13, 200), QGradient(QGradient.Preset.HeavyRain))
        self.headLight2_sound_color_idx = 0
        self.headLight2_state = 0 # 0 -> off 1 -> on    
    ...
    def indicator1_painting(self, painter: QPainter):
        painter.setPen(QPen(self.indicator1_lst[self.indicator1_color_idx], round(self.width()*0.0012), join=Qt.PenJoinStyle.MiterJoin))
        painter.setBrush(QBrush(self.indicator1_lst[self.indicator1_color_idx]))

        indicator1_trans = QTransform()
        indicator1_trans.scale(self.width()*0.0010, self.height()*0.0015)

        indicator1 = QPolygonF((QPointF(40, 80), QPointF(90, 120), QPointF(90, 100), QPointF(150, 100),
                                    QPointF(150, 60), QPointF(90, 60), QPointF(90, 40)))
        
        scaled_indicator1 = indicator1_trans.map(indicator1)
        scaled_indicator1.translate(-scaled_indicator1.boundingRect().x(), -scaled_indicator1.boundingRect().y())
        scaled_indicator1.translate(40, 40)
        painter.drawPolygon(scaled_indicator1)


    def indicator2_painting(self, painter: QPainter):
        painter.setPen(QPen(self.indicator2_lst[self.indicator2_color_idx], round(self.width()*0.0012), join=Qt.PenJoinStyle.MiterJoin))
        painter.setBrush(QBrush(self.indicator2_lst[self.indicator2_color_idx]))
        

        indicator2_trans = QTransform()
        indicator2_trans.scale(self.width()*0.0010, self.height()*0.0015)

        indicator2 = QPolygonF((QPointF(150, 80), QPointF(100, 120), QPointF(100, 100), QPointF(40, 100),
                                    QPointF(40, 60), QPointF(100, 60), QPointF(100, 40)))
        
        scaled_indicator2 = indicator2_trans.map(indicator2)
        scaled_indicator2.translate(-scaled_indicator2.boundingRect().x(), -scaled_indicator2.boundingRect().y())
        scaled_indicator2.translate(1100, 40)
        painter.drawPolygon(scaled_indicator2)

    def headLight_painting(self, painter: QPainter):
        self.start_width = 90
        self.end_width = 118
        self.start_height = (268, 272)
        line_width=5
        painter.setPen(QPen(self.headLight_sound_color_lst[self.headLight_sound_color_idx], line_width))
        for i in range(4):
            # Adjusted starting position for each line
            start_point = QPointF(self.start_width - (i*3), (self.start_height[self.headLight_sound_color_idx] + (i * 15)))
            end_point = QPointF(self.end_width - (i*3), (268 + (i * 15)))
            painter.drawLine(start_point, end_point)

        # Draw the reversed "D" in the center
        d_size = 80  # Adjust the size as needed
        font = painter.font()  # Use painter.font() instead of self.font()
        font.setPixelSize(d_size)
        painter.setFont(font)
        painter.setPen(QPen(self.headLight_sound_color_lst[1], line_width))
        
        

        # Adjusted the position to align with the lines
        painter.drawText(100, 250, d_size, d_size, Qt.AlignCenter, "D")

    def headLight2_painting(self, painter: QPainter):
        self.start_width = 90
        self.end_width = 118
        self.start_height = (268, 272)
        line_width=5
        painter.setPen(QPen(self.headLight2_sound_color_lst[self.headLight2_sound_color_idx], line_width))
        for i in range(4):
            # Adjusted starting position for each line
            start_point = QPointF(self.start_width - (i*3), (self.start_height[0] + (i * 15)))
            end_point = QPointF(self.end_width - (i*3), (268 + (i * 15)))
            painter.drawLine(start_point, end_point)
        ...
        font.setPixelSize(d_size)
        painter.setFont(font)
        painter.setPen(QPen(self.headLight2_sound_color_lst[1], line_width))
    ...
    def set_indicator1_state(self, val):
        self.indicator1_color_idx = val
        if self.indicator1_color_idx != self.indicator1_state:
            self.repaint()
        self.indicator1_state = val
    
    def set_indicator2_state(self, val):
        self.indicator2_color_idx = val
        if self.indicator2_color_idx != self.indicator2_state:
            self.repaint()
        self.indicator2_state = val

    def set_headLight_state(self, val):
        self.headLight_sound_color_idx = val
        if self.headLight_sound_color_idx != self.headLight_state:
            self.repaint()
        self.headLight_state = val

    def set_headLight2_state(self, val):
        self.headLight2_sound_color_idx = val
        if self.headLight2_sound_color_idx != self.headLight2_state:
            self.repaint()
        self.headLight2_state = val    
    ...
    def break_painting(self, painter: QPainter):
        ...
        break_rect = break_fm.boundingRect("100°C")
        painter.setFont(break_font)
        result = self.get_speed()  # Assuming get_speed() returns a tuple
        s, t = result

        break_rect.moveTo(self.rect().center()+QPointF(self.rect().width()*0.345, self.rect().height()*0.1).toPoint())

        painter.setPen(QPen(self.break_color_lst[1], round(self.width()*0.0025)))
        painter.drawText(break_rect, Qt.AlignmentFlag.AlignCenter, f"{t}°C")


        self.line_color = (QColor(67, 13, 13, 200), QGradient(QGradient.Preset.ColorfulPeach))
        line_width = 5

        # Draw four horizontal parallel lines
        start_point = QPointF(90, 250)  # Adjusted starting position
        end_point = QPointF(100, 250)
    ...
    def accelerator_properties(self):
        ...
        self.accelerator_color_lst = (QColor(67, 13, 13, 200), QColor(255, 0, 0, 255))

    def temp_properties(self):
        self.temp_color =  QColor(255, 0, 0, 255)

    def set_speed(self, val, val2):
        self.speed = round(val/self.speed_angle_factor) if round(val/self.speed_angle_factor)<=300 else 300
        self.temp = val2
        self.repaint()

    def get_speed(self):
        return round(self.speed*self.speed_angle_factor), self.temp
    ...
    def temp_painting(self, painter: QPainter):
        ...
        accelerator_rect = accelerator_fm.boundingRect("FIRE WARNING")
        painter.setFont(accelerator_font)
        accelerator_rect.moveTo(self.rect().center()+QPointF(self.rect().width()*0.345, self.rect().height()*0.1).toPoint())
        painter.setPen(QPen(self.accelerator_color_lst[self.accelerator_state], round(self.width()*0.002)))
        painter.drawText(accelerator_rect, Qt.AlignmentFlag.AlignCenter, "FIRE")
    ...
    def speedometer_painting(self, painter: QPainter):
        ...
        painter.drawText(speed_kmph_rect, Qt.AlignmentFlag.AlignCenter, f'{s} km/h')
        # speed
    ...
    def check_all_state_animation(self, loop_count):
        if loop_count == 1:
            ...
            self.set_indicator1_state(1)
            self.set_indicator2_state(1)
            self.set_headLight_state(1)
            self.set_headLight2_state(1)
        elif loop_count == 6:
            self.set_horn_state(0)
            self.set_indicator1_state(0)
            self.set_indicator2_state(0)
            self.set_headLight_state(0)
            self.set_headLight2_state(0)
            self.charge_state = 1
        elif loop_count == 7:
    ...
    def paintEvent(self, event):
        ...
        if self.other_visible:
            ...
            self.indicator1_painting(painter)
            self.indicator2_painting(painter)
            self.headLight_painting(painter)
            self.headLight2_painting(painter)
...
class _DashBoardControls(QObject):
    ...
    set_current_speed_signal = pyqtSignal(int,int)
    set_current_temp_signal = pyqtSignal(int)
    set_speedometer_resetter_sig = pyqtSignal(int)
    break_sig = pyqtSignal(int)
    horn_sig = pyqtSignal(int)
    indicator1_sig = pyqtSignal(int)
    indicator2_sig = pyqtSignal(int)
    headLight_sig = pyqtSignal(int)
    headLight2_sig = pyqtSignal(int)
    ...
    def required_values(self):
        ...
        self.keys_ = {Qt.Key.Key_W: False, Qt.Key.Key_H: False, Qt.Key.Key_D: False, Qt.Key.Key_A: False, Qt.Key.Key_T: False, Qt.Key.Key_Z: False, Qt.Key.Key_Left: False, 
    ...
    def all_connector(self):
        ...
        self.set_current_temp_signal.connect(self.dash_board.dash_board_design_widget.set_temp)
        ...
        self.indicator1_sig.connect(self.dash_board.dash_board_design_widget.set_indicator1_state)
        self.indicator2_sig.connect(self.dash_board.dash_board_design_widget.set_indicator2_state)
        self.headLight_sig.connect(self.dash_board.dash_board_design_widget.set_headLight_state)
        self.headLight2_sig.connect(self.dash_board.dash_board_design_widget.set_headLight2_state)
    ...
    def set_temp(self, current_temp):
        self.set_current_temp_signal.emit(current_temp)

    def set_speed(self, current_speed, current_temp ):
        self.set_current_speed_signal.emit(current_speed, current_temp)
    ...
    def on_indicator1(self):
        self.keys_[Qt.Key.Key_A] = True
        self.indicator1_sig.emit(1)
    
    def on_indicator2(self):
        self.keys_[Qt.Key.Key_D] = True
        self.indicator2_sig.emit(1)

    def on_headLight(self):
        self.keys_[Qt.Key.Key_T] = True
        self.headLight_sig.emit(1)

    def on_headLight2(self):
        self.keys_[Qt.Key.Key_Z] = True
        self.headLight2_sig.emit(1)    

    def off_horn(self):
        self.keys_[Qt.Key.Key_H] = False
        self.horn_sig.emit(0)

    def off_indicator1(self):
        self.keys_[Qt.Key.Key_A] = False
        self.indicator1_sig.emit(0)
    
    def off_indicator2(self):
        self.keys_[Qt.Key.Key_D] = False
        self.indicator2_sig.emit(0)

    def off_headLight(self):
        self.keys_[Qt.Key.Key_T] = False
        self.headLight_sig.emit(0)

    def off_headLight2(self):
        self.keys_[Qt.Key.Key_Z] = False
        self.headLight2_sig.emit(0)    
...
class TriggerAction():
    ...
    def set_speed(self, current_speed: int, current_temp: int):
        ...
        speedometer update"""
        self.__dbc.set_speed(current_speed, current_temp)
    ...
    def on_indicator1(self):
        """To activate indicator1"""
        self.__dbc.on_indicator1()
    
    def on_indicator2(self):
        """To activate indicator2"""
        self.__dbc.on_indicator2()

    def on_headLight(self):
        """To activate headLight"""
        self.__dbc.on_headLight()

    def on_headLight2(self):
        """To activate headLight"""
        self.__dbc.on_headLight2()    
    ...
    def off_indicator1(self):
        """To deactivate indicator1"""
        self.__dbc.off_indicator1()
    
    def off_indicator2(self):
        """To deactivate indicator2"""
        self.__dbc.off_indicator2()

    def off_headLight(self):
        """To deactivate headLight"""
        self.__dbc.off_headLight()

    def off_headLight2(self):
        """To deactivate headLight"""
        self.__dbc.off_headLight2()    
